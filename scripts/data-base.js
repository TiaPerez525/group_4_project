class DataBase {
    ...
    Awake() {
        var displayingProductIndex = window.localStorage.getItem(DISPLAY_PRODUCT_INDEX_SAVE_KEY);

        if (displayingProductIndex != null) 
            displayingProductIndex = displayingProductIndex;
    }

    AddToFrontLink(_addingDirectiory) {
        for (var key in DataBase_productList) {
            var product = DataBase_productList[key];
            product.imageLink = _addingDirectiory + product.imageLink;
        }
    }

    HandleSetDisplayingProductIndex(_index) {
        displayingProductIndex = _index;
        window.localStorage.setItem(DISPLAY_PRODUCT_INDEX_SAVE_KEY, _index)
    }

    GetProductListLength() {
        return Object.keys(DataBase_productList).length
    }
}